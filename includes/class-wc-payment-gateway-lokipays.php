...
class WC_Gateway_Lokipays extends WC_Payment_Gateway {
	...
	private function lokipays_payment_processing( $order ) {
		...
		$url = 'https://lokipays.com/api/Transaction/CardDeposit';
		// var_dump($order_id);
		$body = array(
			...
			'creditCard' => array( 
				...
				'expiryYear' => $_POST['expiry_year_on_card'],
		...
		if ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			$order->update_status( apply_filters( 'woocommerce_lokipays_process_payment_order_status', $order->has_downloadable_item() ? 'wc-invoiced' : 'processing', $order ), __( 'Payments pending.', 'lokipays-payments-woo' ) );
		}

		if ( 200 === wp_remote_retrieve_response_code( $response ) ) {
			$res_body = wp_remote_retrieve_body( $response );
			// var_dump($response_body['message']);
			$response_body = array('message', "Thank you! Your payment was successful");
